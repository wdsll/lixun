...
int main(void)
{
	...
	/* Initialize the Gpt driver */
       ...
       eReturnValue |= Sbc_fs23_ReadRegister(SBC_FS23_M_WU1_FLG_ADDR,&RxDataflag);

       eINITState = Sbc_fs23_CheckInitState();

       eNormalState = Sbc_fs23_CheckNormalState();

       eDebugState = Sbc_fs23_CheckDebugState();

       Sbc_fs23_InitDriver(NULL_PTR);
       eReturnValue = Sbc_fs23_GoToInitState();
       eReturnValue |= Sbc_fs23_InitDevice();
       /* Turn on V2 regulator again*/
	   Vreg.bV2Enable = TRUE;
	   eReturnValue |= Sbc_fs23_SetRegulatorState(Vreg);
	   eReturnValue |= Sbc_fs23_FsOutputRelease(SBC_FS23_FS_FS0B);
	   ...
	   for(Index = 0; Index <= 5; Index++)
		   ...
		   while(1)
		   {
			   ...
			   if((RxDataflag & SBC_FS23_M_WD_OFL_WU_MASK)==SBC_FS23_M_WD_OFL_WU_MASK)
				 {
				   printf("SBC_FS23_M_WU1_FLG_ADDR-RxDataflag:%d\r\r\n\n\n",RxDataflag);
				  // eReturnValue |= Sbc_fs23_WriteRegister(((uint8)(SBC_FS23_M_WU1_FLG_ADDR)), (RxDataflag|(uint16)SBC_FS23_M_WD_OFL_WU_MASK));
					printf("Wake-up by watchdog max error failure occurred！ \r\n");
					printf(" WD_OFL_WU=1 ！ \r\n");
					//Dio_WriteChannel(DioConf_DioChannel_LED_R, STD_HIGH);
				 }
			   ...
			   eINITState = Sbc_fs23_CheckInitState();
			   if (eINITState == SBC_Init_INIT_STATE)
				{
					//printf("FS23 STATE is INIT STATE ！ \r\n");
				}
				else if (eINITState == SBC_Init_NON_INIT_STATE)
				{
					//printf("FS23 STATE is Not INIT STATE ！ \r\n");
				}
				else
				  {
					//printf("FS23 STATE is INVALID ！ \r\n");
			   ...
			   eNormalState = Sbc_fs23_CheckNormalState();
				if(eNormalState == SBC_FS23_NORMAL_STATE)
				{
					//printf("FS23 STATE is NORMAL ！ \r\n");

				}
				else if(eNormalState == SBC_FS23_NON_NORMAL_STATE)
				{

					//printf("FS23 STATE is NOT NORMAL ！ \r\n");
				}
				else
				{
					//printf("FS23 STATE is INVALID ！ \r\n");
				...
				if(eDebugState == SBC_FS23_DEBUG_STATE)
				{
					//printf("FS23 STATE is DEBUG ！ \r\n");

				}
				else if(eDebugState == SBC_FS23_NOT_DEBUG_STATE)
				{

					//printf("FS23 STATE is NOT DEBUG ！ \r\n");
				}
				else
				{
					//printf("FS23 STATE is Invalid ！ \r\n");
				...
				if((eINITState == SBC_Init_INIT_STATE)&&(eNormalState == SBC_FS23_NORMAL_STATE))
				{
					...
					eReturnValue |= Sbc_fs23_gotoNormal();
					eReturnValue |=Sbc_fs23_SetWakeupclose(0); //关闭唤醒
				...
				eReturnValue |= Sbc_fs23_ReadRegister(SBC_FS23_M_IOWU_FLG_ADDR,&RxDataflag);
				if((RxDataflag & 0x0003u))
				{
				eReturnValue |= Sbc_fs23_ReadRegister(SBC_FS23_M_IO_TIMER_FLG_ADDR,&RxDataflag);
				eReturnValue |= Sbc_fs23_WriteRegister(((uint8)(SBC_FS23_M_IO_TIMER_FLG_ADDR)), (RxDataflag|(uint16)0x0003U));
				eReturnValue |= Sbc_fs23_ReadRegister(SBC_FS23_M_IOWU_FLG_ADDR,&RxDataflag);
				eReturnValue |= Sbc_fs23_WriteRegister(((uint8)(SBC_FS23_M_IOWU_FLG_ADDR)), (RxDataflag|(uint16)0x0003U));
				//eReturnValue |=Sbc_fs23_SetWakeupOpen(0); //打开唤醒
			   ...
			   if(ADflag = 1)
			   {
				   eReturnVal =Sbc_fs23_SetAmux(SBC_FS23_M_AMUX_V1,SBC_FS23_M_AMUX_DIV_LOW);
				   CHANNUM=1;
				   // 启用 ADC 转换结束的通知功能，包括正常转换链结束和注入转换链结束
void Sbc_fs23_TimeDelay(uint32 DelayUs)
{
    uint32 deltaTime = 0U;
    uint32 timeoutTick = OsIf_MicrosToTicks(DelayUs, OSIF_COUNTER_DUMMY);
    ...
    while (deltaTime <= timeoutTick)
    {
        uint32 elapsed = OsIf_GetElapsed(&startTime, OSIF_COUNTER_DUMMY);
        // 检查 elapsed 是否为有效值
        if (elapsed == 0xFFFFFFFFU || elapsed > timeoutTick) {  // 假设 0xFFFFFFFFU 是无效值
            break;  // 如果 elapsed 无效，退出循环
        }
        deltaTime += elapsed;

        // 防止整数溢出
        if (deltaTime < elapsed) {  // 如果 deltaTime 小于 elapsed，说明发生了溢出
            break;  // 退出循环
        }
    }
}
			...
			if(fIndex==0)
			{
						...
						printf("Init-FS_SAFETY_FLG:%d\r\r\n\n\n",RxDataflag1);

						printf("Init-M_IOWU_FLG:%d\r\r\n\n\n",RxDataflag2);

						printf("Init-M_WU1_FLG:%d\r\r\n\n\n",RxDataflag3);

						printf("Init-M_REG_FLG:%d\r\r\n\n\n",RxDataflag4);

			}

			 if(fIndex!=3)
			 {
				eReturnValue |= Sbc_fs23_ReadRegister(SBC_FS23_FS_SAFETY_FLG_ADDR,&RxDataflag);
				printf("While1-FS_SAFETY_FLG:%d\r\r\n\n\n",RxDataflag);
				eReturnValue |= Sbc_fs23_ReadRegister(SBC_FS23_M_IOWU_FLG_ADDR,&RxDataflag);
				printf("While1-M_IOWU_FLG:%d\r\r\n\n\n",RxDataflag);
				eReturnValue |= Sbc_fs23_ReadRegister(SBC_FS23_M_WU1_FLG_ADDR,&RxDataflag);
				printf("While1-M_WU1_FLG:%d\r\r\n\n\n",RxDataflag);
				eReturnValue |= Sbc_fs23_ReadRegister(SBC_FS23_M_REG_FLG_ADDR,&RxDataflag);
				printf("While1-M_REG_FLG:%d\r\r\n\n\n",RxDataflag);
				fIndex++;
			 }
}
